...
def extract_candidates(img_file: str, candidates: pd.DataFrame, annotations: pd.DataFrame, window: int = 20) -> Tuple[np.ndarray, np.ndarray, float]:
    """
    Extracts the region of interest from the CT scan.
    ...
    logging.info('{} are true nodules (class 1) and {} are non-nodules (class 0)'.format(numNodules, numNonNodules))
