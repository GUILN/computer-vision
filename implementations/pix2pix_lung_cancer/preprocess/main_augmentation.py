...
def process_images(data_dir: str, rotate_degrees: int):
    ...
    logging.info("Total number of rotations: %d ((div 360) - 1)", total_num_rotations)
    logging.info(
        "Total number of images after augmentation: %d",
        len(files) * total_num_rotations,
    )
...
def main():
    ...
    parsert.add_argument(
        "--data_dir", type=str, help="data dir to with images to augment"
    )
    parsert.add_argument(
        "--rotate_degrees",
        type=int,
        default=DEFAULT_DEGREES,
        help=f"degrees to roate images in order to create more images - default {DEFAULT_DEGREES} degrees",
    )
